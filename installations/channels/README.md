# `finix` Channel Installation Guide

This guide is for installing Finix using a *channel-based* workflow. This directory contains a 
preset `configuration.nix` for configuring everything needed to boot into a TTY environment with
network access.
...
Lastly for configuration, customize the `user.user.<USERNAME>` entry to match your
preferences. `<USERNAME>` should be changed to your username, `password` is a
